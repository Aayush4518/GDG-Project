...
export const api = {
  ...
  postLocation: (touristId: string, body: { latitude: number; longitude: number; timestamp: string }) =>
    http(`/tourists/${touristId}/location`, { method: 'POST', body: JSON.stringify(body) }),
  postPanic: (touristId: string, body: { latitude: number; longitude: number; timestamp: string; message?: string }) =>
    http(`/tourists/${touristId}/panic`, { method: 'POST', body: JSON.stringify(body) }),
};

export const wsUrl = () => {
  const host = (process as any).env?.EXPO_PUBLIC_WS_HOST || (global as any).EXPO_PUBLIC_WS_HOST || 'localhost:8000';
  return `ws://${host}/ws/dashboard`;
};
